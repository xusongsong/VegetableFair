...
var Map3D = OM.Class.extend({
        ...
        "initialize" : function(option) {
            ...
            try {
                ...
                try{
					//IE 8 、IE9 、IE 10 浏览器的支持的插件初始化方式
                    content3d = document.createElement("object");
                    document.getElementById(this.option.id).appendChild(content3d);
                    content3d.setAttribute("id","VPSDKCtrl");
                    content3d.style.width = this.option.width || "100%";
                    content3d.style.height = this.option.height || "100%";
                    content3d.classid = this.option.classid
                        || "CLSID:E6F71384-437A-48ED-8E01-CD280CADF44F";
                }catch (e1) {
                    alert("请利用容器DIV创建地图");
        ...
        "sightAnalysis" : function(VAngle, HAngle) {
        	var VAngle = VAngle || "60";
        	var HAngle = HAngle || "60";
            //对空间互斥图层进行互斥处理
...
//				resp.AddConfig("Visible", "false");
        ...
        "createRoamMode" : function(type) {
            ...
            switch (type) {
                case "OnWalk":
                    navagation.SetCustomGlideRoamMode(true, 35, 0.3, -8);//是否开启漫游//漫游高度//漫游速度//漫游角度
                    break;
                case "OnDrive":
                    navagation.SetCustomGlideRoamMode(true, 35, 4.5, -8);
                    break;
                case "OnFly":
        ...
        "addRoamPathOld":function(coordStr,viewModel,state){
            ...
            tlo.AddConfig("ViewObjectMode","0.0,-0.838920733233321,50.0");
            tlo.AddConfig("KeyPoints", coordStr);
        ...
        /* 添加新文字标注要素 */
		"addPointTextLabel2" : function(opt) {
			this.opt = opt;
			var layer = this.opt.layer;
            ...
            var Lat = this.opt.Lat || '';
            var Height = this.opt.Height || '';
            var textValue = this.opt.textValue || '';
            var ename = this.opt.ename;
			var id = layer.GetLayerID(); // 获取图层id
			var editLayer = map.GetFeatureModelLayer(id); // 获取矢量图层
			/* 创建文字要素 */
			var addFeature = map.CreateFeature(); // 创建要素对象
        ...
        "CreatePointTextEditLayerfunction":function(opt) {
                ...
                var lineFlage = this.opt.lineFlage || 'false';
                var pSymbol = map.CreateSymbol("PointSymbol"); ////创建类型为PointSymbol的符号，必须为PointSymbol字符串
                pSymbol.AddConfig("Size", "10"); ////点大小，范围0-10
                pSymbol.AddConfig("Color", "1.0,1.0,0.0,1.0"); ////颜色值0-1（RGBA），最后一位代表透明度，0为透明，1为不透
                /////////////////////此部分是文字在场景中显示的配置/////////////////
                ...
                tSymbol.AddConfig("FillingColor", "255, 255, 255, 1.0"); ////文字颜色（RGBA），颜色值0-1，最后一位代表透明度，0为透明，1为不透
                tSymbol.AddConfig("Font", fontPath); ////文字字体，从系统字体目录中取，字体文件必须存在，配置一些参数时，如果没生效可能与字体文件相关，例如中文
                tSymbol.AddConfig("Size", "40"); ////字体精度大小
                tSymbol.AddConfig("CharacterSize", "8"); ////文字大小
                ...
                tSymbol.AddConfig("LineColor", "0.6,0.6,0.6,1.0");      //接地线颜色
                tSymbol.AddConfig("IsAddGroundLine", lineFlage);           //是否开启接地线
                tSymbol.AddConfig("FeatureLiftUp", liftUp);                 //接地线抬升值(配置该项接地线将是文字到点之间，否则是文字、点到地底)
                tSymbol.AddConfig("Content", "[NAME]");                 //[]里代表矢量的某字段名称
                //***********如果需要配置背景图片，则需要添加图片资源库*********/
                ...
                tlo.AddConfig("FeatureSourceType", "ogr"); ////要素数据源类型，针对shp、dxf数据源必须是ogr
                tlo.AddConfig("Fields", "Name:String:100:0,textname:String:100:0,lon:String:100:0,lat:String:100:0,height:String:100:0,ename:String:100:0"); ////创建矢量的属性字段，属性名：属性类型：类型长度：小数点后几位
                tlo.AddConfig("GeometryType", "Point"); ////几何类型     Point为点 Polyline为线 Polygon为面 此项配置不能少或字符串一定不能错误，否则保存文件不成功
                tlo.AddConfig("TileSizeFactor", "1.0"); ////瓦片大小的影响因子，建议是1.0
                tlo.AddConfig("TileSize", "0"); ////瓦片大小，根据数据实际情况设置，根据数据面积来，面积越大值越大
                tlo.AddConfig("LiftUp", "0"); ////抬升高度，任意值
            ...
            //********************************
            //以矢量点数据为例：
             "CreatePointAndIcon":function (opt) {
	    	    this.opt = opt;
	            var fontPath = content3d.GetSDKPath().replace("\\bin", "");
	            fontPath += "data\\Fonts\\msyh.ttf";// SDK路径下的字体
	            var backgroundUrl = this.opt.backgroundUrl;
	            var Align = this.opt.Align || "5";
                /////////////////////此部分是点在场景中显示的配置/////////////////
                var pSymbol = map.CreateSymbol("PointSymbol"); 							//// 创建类型为PointSymbol的符号，必须为PointSymbol字符串
                ...
                tSymbol.AddConfig("XScale", "0.35"); 									//// 图片x方向放大比例
                tSymbol.AddConfig("YScale", "0.35"); 									//// 图片y方向放大比例
                tSymbol.AddConfig("ZScale", "0.35"); 									//// 图片z方向放大比例
                ...
                tSymbol.AddConfig("Url", backgroundUrl); ////图标资源路径
                tSymbol.AddConfig("LibraryName", "reslib"); 							//// 资源名称
                ...
                tlo.AddConfig("Url", "D:\\E\\hh\\测试数据\\演示测试数据\\point.shp"); ////数据存放位置，注意双斜杠
                tlo.AddConfig("FeatureSourceType", "ogr"); 								//// 要素数据源类型，针对shp、dxf数据源必须是ogr
                tlo.AddConfig("TileSizeFactor", "1.0"); 								//// 瓦片大小的影响因子，建议是1.0
                tlo.AddConfig("TileSize", "5000"); 										//// 瓦片大小，根据数据实际情况设置，根据数据面积来，面积越大值越大
        ...
        "degreeToCgcs_2000" : function(lon,Lat){
            //经纬度转三维坐标
            return DegreetoMeter_1984(lon,Lat);
        },
        "cgcs_2000ToDegree" : function(x,y){
            //三维场景坐标转经纬度
            return MeterToDegree_1984(x,y);
        },
        "VideoAreaCreate" : function(){
            var tlo = map.CreateLayerOptions("vArea"); 					// 创建分析图层配置，给配置起个名称，任意名称
            tlo.AddConfig("LayerOptionsName", "AnalysisLayerOptions"); 	// 创建配置类型, AnalysisLayerOptions代表分析图层数据配置，必须是此键值对
            tlo.AddConfig( "DataSourceTypeName", "as_videoarea" ); 		// 此处格式固定，使用区域投影插件
            //tlo.AddConfig("VideoResources", "share2233");// 共享内存方式
            var videoArea = map.CreateLayer("AnalysisLayer", tlo); 			// 创建分析图层，第一项参数必须为AnalysisLayer
            videoArea.AddObserver();
            map.AddLayer(videoArea);
        ...
        "loadResource" : function(video,points,id,resUrl,imgtype,nodepos){
            ...
            tlo.AddConfig( "ID",id);
            tlo.AddConfig( "AreaPoints",points);
            tlo.AddConfig( "TransFormNodePos", nodepos);

            tlo.AddConfig( "LineWidth", "5");
            tlo.AddConfig( "LineColor", "0.300000011921,0.300000011921,0.40000000596,1");
            tlo.AddConfig( "LineIsVisible", "false" );


            tlo.AddConfig( "ImageIsVisible", "true");
            tlo.AddConfig( "ImageFitType", imgtype );

            tlo.AddConfig( "LoadWaitImagePath", SDKpath+"data\\\\texture\\\\loading.jpg" );
            tlo.AddConfig( "VideoResources", resUrl); // 视频路径

            video.UpdateLayerOptions(tlo);
        },
